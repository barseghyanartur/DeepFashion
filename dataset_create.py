...
category_name_generate = ['Anorak', 'Blazer', 'Blouse', 'Bomber', 'Button-Down',
                          ...
                          'Jeans', 'Jeggings', 'Jodhpurs', 'Joggers',
                          'Leggings', 'Sarong', 'Shorts', 'Skirt', 'Sweatpants',
                          'Sweatshorts', 'Trunks', 'Caftan', 'Cape', 'Coat',
                          'Coverup', 'Dress', 'Jumpsuit', 'Kaftan', 'Kimono',
...
# category_name_generate = ['Skirt', 'Top', 'Jeans', 'Joggers', 'Hoodie',
#                           'Sweatpants', 'Coat', 'Sweatshorts', 'Capris']
#
# category_name_generate = ['Jeggings', 'Kaftan', 'Anorak', 'Flannel', 'Robe',
#                           'Chinos', 'Parka', 'Jersey', 'Poncho', 'Trunks',
#                           'Peacoat', 'Turtleneck', 'Button-Down', 'Capris',
#                           'Bomber', 'Coat', 'Sweatshorts', 'Jeans', 'Hoodie']
...
def get_category_names():
    category_names = []
    with open(
            fashion_dataset_path + '/Anno/list_category_cloth.txt') as file_list_category_cloth:
...
def display_bbox(image_path_name, boxA, boxB):
    ...
    rect = mpatches.Rectangle((x, y), w, h, fill=False, edgecolor='green',
                              linewidth=1)
    ax.add_patch(rect)
...
def calculate_bbox_score_and_save_img(image_path_name, dataset_image_path,
    ...
    logging.debug(
        '{} {} {}'.format(img_read.format, img_read.size, img_read.mode))
    ...
    for x, y, w, h in (candidates):
        ...
        image_save_name = image_path_name.split('/')[-2] + '_' + \
                          image_path_name.split('/')[-1].split('.')[0]
        image_save_path = dataset_image_path.rsplit('/', 1)[0]
        image_save_path_name = image_save_path + '/' + image_save_name + '_crop_' + str(
            x) + '-' + str(y) + '-' + str(x + w) + '-' + str(
    ...
    image_save_name = image_path_name.split('/')[-2] + '_' + \
                      image_path_name.split('/')[-1].split('.')[0]
    image_save_path = dataset_image_path.rsplit('/', 1)[0]
    image_save_path_name = image_save_path + '/' + image_save_name + '_gt_' + str(
        gt_x1) + '-' + str(gt_y1) + '-' + str(gt_x2) + '-' + str(
        gt_y2) + '_iou_' + '1.0' + '.jpg'
    logging.debug('image_save_path_name {}'.format(image_save_path_name))
...
def generate_dataset_images(category_names, continue_from=None):
    ...
    with open(
        ...
        with open(
                fashion_dataset_path + '/Anno/list_category_img.txt') as file_list_category_img:
            with open(fashion_dataset_path + '/Eval/list_eval_partition.txt',
                      'r') as file_list_eval_ptr:
                ...
                for line in file_list_category_img:
                    ...
                    logging.debug('image_name {}'.format(
                        image_name))  # image_name img_00000051.jpg
                    image_full_name = line[0].replace('/', '_')
                    logging.debug('image_full_name {}'.format(
                        image_full_name))  # img_Tailored_Woven_Blazer_img_00000051.jpg
                    image_category_index = int(line[1:][0]) - 1
                    logging.debug('image_category_index {}'.format(
                        image_category_index))  # 2

                    if category_names[
                        image_category_index] not in category_name_generate:
                        logging.debug('Skipping {} {}'.format(
                            category_names[image_category_index],
                            image_path_name))
                        continue
                    ...
                    if image_category_index < max_categories:
                        ...
                        dataset_image_path = ''
                        dataset_split_name = get_dataset_split_name(
                            image_path_name, file_list_eval_ptr)
                        ...
                        if dataset_split_name == "train":
                            ...
                            dataset_image_path = os.path.join(dataset_val_path,
                                                              category_names[
                                                                  image_category_index],
                                                              image_full_name)
                        elif dataset_split_name == "test":
                            dataset_image_path = os.path.join(dataset_test_path,
                                                              category_names[
                                                                  image_category_index],
                                                              image_full_name)
                        else:
                            logging.error(
                                'Unknown dataset_split_name {}'.format(
                                    dataset_image_path))
                            exit(1)

                        logging.debug('image_category_index {}'.format(
                            image_category_index))
                        logging.debug('category_names {}'.format(
                            category_names[image_category_index]))
                        logging.debug(
                            'dataset_image_path {}'.format(dataset_image_path))

                        # Get ground-truth bounding boxes
                        gt_x1, gt_y1, gt_x2, gt_y2 = get_gt_bbox(
                            image_path_name,
                            file_list_bbox_ptr)  # Origin is top left, x1 is distance from y axis;
                        # x1,y1: top left coordinate of crop; x2,y2: bottom right coordinate of crop
                        logging.debug(
                            'Ground bbox:  gt_x1:{} gt_y1:{} gt_x2:{} gt_y2:{}'.format(
                                gt_x1, gt_y1, gt_x2, gt_y2))

                        image_path_name_src = os.path.join(fashion_dataset_path,
                                                           'Img',
                                                           image_path_name)
                        logging.debug('image_path_name_src {}'.format(
                            image_path_name_src))

                        calculate_bbox_score_and_save_img(image_path_name_src,
                                                          dataset_image_path,
                                                          gt_x1, gt_y1, gt_x2,
                                                          gt_y2)
                    ...
                    count = count + 1
                    logging.info(
                        'count {} {}'.format(count, dataset_image_path))
...
def display_category_data():
    for path in [dataset_train_path, dataset_val_path, dataset_test_path]:
        ...
        for dirs1_name in dirs1:
            path2, dirs2, files2 = os.walk(
                os.path.join(path, dirs1_name)).next()
